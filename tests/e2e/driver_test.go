...
import (
	...
	"github.com/racktopsystems/brickstor-csi-driver/tests/utils/k8s"
	"github.com/racktopsystems/brickstor-csi-driver/tests/utils/remote"
)

const (
	defaultSecretName = "brickstor-csi-driver-config"
)
...
func GenerateConfigFunc(configType, id, aclRule, zone, bsrBox string) (string, error) {
	...
	if configType == "dynamic" {
		...
		var dataForCfg = forPersistent{bsrBox}
		fileCfg, err := os.Create(configPath)
		if err != nil {
			return "", errors.New("cannot create config file")
		}
...
func TestDriver_deploy(t *testing.T) {
	...
	installed := t.Run("install driver", func(t *testing.T) {
		...
		waitPods := []string{
			"brickstor-csi-controller-.*Running",
			"brickstor-csi-node-.*Running",
		}
	...
	t.Run("deploy nginx pods using several appliances in one config file", func(t *testing.T) {
		...
		pathConfigMultiNs1, err := GenerateConfigFunc("persistent", "1", "", "", "brickstor1:pool1/global/data/csi/persistent")
		if err != nil {
		...
		pathConfigMultiNs2, err := GenerateConfigFunc("persistent", "2", "", "", "brickstor2:p02/global/data/csi/persistent")
		if err != nil {
			if _, err := client.AddResultForCase(5151, 801250, testResult); err != nil {
				l.Warn("Can't add test result to TestRail")
			}
	...
	t.Run("uninstall driver", func(t *testing.T) {
		...
		if err := k8sDriver.Delete([]string{"brickstor-csi-.*"}); err != nil {
			testResult.StatusID = 5
